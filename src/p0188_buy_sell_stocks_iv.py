...
def max_profit_dp_helper(
    ...
    while k <= max_transactions:
        for interval in range(len(price_intervals)):
            max_profit = 0
            for left in range(interval, len(price_intervals)):
                for right in range(left, len(price_intervals)):
                    if price_intervals[right][1] > price_intervals[left][0]:
                        ...
                        if k > 1:
                            if right + 1 in curr_profits:
                                profit += curr_profits[right + 1]
                        if profit > max_profit:
                            max_profit = profit
            next_profits[interval] = max_profit
